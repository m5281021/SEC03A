...
def register_new_movie_seats(request):
    try:
        with transaction.atomic():
            ...
            seat = Seat(showtime = showtime_number, reserved = True, user = user_name)
            seat.save()
...
def show_user_movies(request):
    try:
        with transaction.atomic():
            movie = Movie.objects.filter(user__name = user_name)
            print(movie)
...
def show_matching_movies_date(request):
    try:
        with transaction.atomic():
            show_movie = Showtime.objects.filter(times = date)
            print(show_movie)
